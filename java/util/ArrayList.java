 ...
 * 随着元素被不断地加入ArrayList中，它地容量能够动态增长
 ...
 *
 * <p><a name="fail-fast">
 * Arraylist返回的iterator或listIterator迭代器对象具有快速失败机制,fail-fast，
 * 若在iterator创建之后的任意时间Arraylist以任何除去迭代器ListIterator本身的remove, add等被修改，iterator将会抛出ConcurrentModificationException异常，
 * 为了应对并发修改，iterator快速失败，而非隐藏风险。
        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    /**
     * 这个数组缓冲区中存储ArrayList的元素，ArrayList的容量就是这个array buffer即数组的长度。
     * 任何空的ArrayList elementData == DEFAULTCAPACITY_EMPTY_EMEMENTDATA当第一个元素被添加进来的时候将被扩展到默认容量大小
    ...
    private void ensureExplicitCapacity(int minCapacity) {
        modCount++;
        // 若默认容量小于size+1，即添加元素时需要占用一个位置，size+1就是当前共需要多少个，查看elementData数组大小是否足够放置size+1个元素
        // 若size+1 > elementData.length说明不够用了，要扩容
     ...
     * @param minCapacity the desired minimum capacity
     */
    private void grow(int minCapacity) {  //minCapicity+1
        // overflow-conscious code
